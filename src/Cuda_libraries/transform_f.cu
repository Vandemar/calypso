...
// 3 for m-1, m, m+1
  ...
  for(int t=1; t<=nScalar; t++) {
    for(int counter = 0; counter < ITEMS_PER_THREAD; counter ++) {
      l_rtm = blockDim.x*counter + threadIdx.x; 
      ip_rtm = t + 3*nVector + nComp * (l_rtm * istep_rtm_t + k_rtm * istep_rtm_r + (mdx_p-1)*istep_rtm_m); 
      idx = idx_p_rtm + l_rtm; 
...
//  static int nShells = *ked - *kst + 1;
  ...
  static Timer transF_s("Fwd scalar reduction algorithm 64 threads 6 Items");
  cudaPerformance.registerTimer(&transF_s);
  transF_s.startTimer();
  transF_scalar_reduction <64, 4, 
                     cub::BLOCK_REDUCE_RAKING_COMMUTATIVE_ONLY,
                     double>
               <<<grid, 64>>> (deviceInput.vr_rtm, deviceInput.sp_rlm, deviceInput.weight_rtm, deviceInput.mdx_p_rlm_rtm, deviceInput.p_rtm, deviceInput.g_sph_rlm_7, constants);
  cudaDevSync();
  transF_s.endTimer();
  ...
  static Timer transf_reduce_32_3("fwd vector reduction algorithm 32 threads/block");
  cudaPerformance.registerTimer(&transf_reduce_32_3);
  transf_reduce_32_3.startTimer();
  transF_vec_reduction< 64, 4,
                  cub::BLOCK_REDUCE_RAKING_COMMUTATIVE_ONLY,
                      double>
            <<<grid, 64>>> (deviceInput.idx_gl_1d_rlm_j, deviceInput.vr_rtm, deviceInput.sp_rlm, deviceInput.radius_1d_rlm_r, 
